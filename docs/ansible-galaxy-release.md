...
This runbook describes how to publish the standalone
`inviqa.digitalocean_reserved_ip`
Ansible role to Ansible Galaxy after a GitHub release is ready.

Use the current release version wherever the examples show `0.2.0`.
...
For major releases, include any breaking changes and upgrade-guide links in the
GitHub release body.
...
```bash
RELEASE_VERSION=0.2.0 ws github release check
RELEASE_VERSION=0.2.0 ws github release publish
```
...
- namespace: `inviqa`
- GitHub repository: `inviqa/ansible-digitalocean-reserved-ip`
- branch: `main`
- Galaxy role name: `digitalocean_reserved_ip`

The command also prints
`ansible-galaxy role info inviqa.digitalocean_reserved_ip` after a successful
import.
...
| --- | --- | --- |
| `inviqa-ansible-roles-releases` | Secret text | Creates the GitHub release in `inviqa/ansible-digitalocean-reserved-ip`. |
| `ansible-digitalocean-reserved-ip-galaxy-token` | Secret text | Imports the role into Ansible Galaxy. Prefer a token loaded by a dedicated Galaxy publishing account rather than a personal maintainer account. |
...
The Galaxy stage resolves the same version, checks the GitHub release state,
exits without importing if the version is already visible on Galaxy, otherwise
imports the role and verifies the version with a pinned Galaxy install.
...
version="0.2.0"
tmp_dir="$(mktemp -d)"
ansible-galaxy role install --roles-path "${tmp_dir}" "inviqa.digitalocean_reserved_ip,${version}"
find "${tmp_dir}" -maxdepth 2 -type f -name main.yml
rm -rf "${tmp_dir}"
```

For future releases, replace `0.2.0` with the release tag.
...
- If Jenkins starts failing after a Galaxy token was reloaded in the UI, update
  the `ansible-digitalocean-reserved-ip-galaxy-token` Secret text credential.
  Galaxy invalidates the previous token when a new one is loaded.
